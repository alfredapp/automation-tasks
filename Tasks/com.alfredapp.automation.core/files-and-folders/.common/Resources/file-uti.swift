...
// Read
if (writeMode == .read) {
  alfredArgs(allPaths.map { fileType($0).identifier })
  exit(EXIT_SUCCESS)
}

// Filter
guard let typeFilter: UTType = UTType(arguments[2]) else { fatalError("Unrecognised type: \(arguments[2])") }

switch writeMode {
case .filter:  alfredArgs(allPaths.compactMap { fileType($0).conforms(to: typeFilter) ? $0.path : nil     })
case .xfilter: alfredArgs(allPaths.compactMap { fileType($0).conforms(to: typeFilter) ? nil     : $0.path })
